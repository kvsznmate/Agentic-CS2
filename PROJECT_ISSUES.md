...
- **(1) Feasibility — ✅ DONE, CONFIRMED ON-MACHINE (2026-08-19).** `src/gsi_probe.py` (now a thin harness over the shared listener, D-031; stamps every GSI POST with `perf_counter` — the recorder's frame clock — prints alive/dead + weapon + phase on change; logs all payloads to `data/gsi_probe/*.jsonl`) + `gsi/gamestate_integration_agenticcs2.cfg` (subscribes `provider`/`map`/`round`/`player_id`/`player_state`/`player_weapons`) + `gsi/README.md`. Ran against a local game: local `player_state` seen, health tracked reality, death edge fired, weapon/ammo came through, die-and-spectate transition observed. `extract_state()` parsing unit-tested against alive/dead/spectating/warmup/weapon payloads. **GSI confirmed alive on CS2 in 2026** (web-verified + maintained third-party CS2 GSI libs) — unlike the dead RAM-offset path (D-002/D-015).
- **(2) DATA_FORMAT change — ✅ BUILT (D-031).** Format bumped **v3 → v4**: per-frame `alive` (uint8 0/1) + `round_phase` (`<U16`) arrays added, index-aligned like every other per-frame array; DATA_FORMAT.md updated (schema table, manifest example, back-compat note). The recorder writes them; the loader reads them; v1/v2/v3 stay readable (they simply lack the fields).
- **(3) Recorder integration + loader filter — ✅ BUILT (D-031).** The design problem (GSI is push/throttled event-driven HTTP; the capture loop is deliberately single-threaded, D-015 "way one") was solved exactly as D-030 anticipated: a shared **`src/gsi_listener.py`** runs the HTTP receiver on its OWN thread (the `raw_mouse`/D-019 isolated-thread pattern) and the capture loop SAMPLES its latest-value slot once per frame (`Recorder._sample_gsi`), so the sync path stays fully synchronous. The flag is **forward-filled** between updates for free (the slot holds the last state). The alive rule is **own-POV health>0** (spectating a living teammate → DEAD, the load-bearing guard). Recording won't start until GSI's first POST (a **liveness gate**: refuses with a setup checklist on timeout, so a mis-configured GSI can't yield a silent zero-frame session), so there are no pre-contact frames. Loader side reuses D-026's opt-in structural exclusion: `SessionDataset`/`build_datasets(use_gameplay_filter=True)` serve only `alive==1` frames, ANDed with the variance keep-mask if both are on; **default OFF** so the #7 gate and split never move. New `python -m src.recorder --verify-gsi` and loader `--gameplay-filter`.
- **On-machine verification of the v4 path — ☐ REMAINING.** All four files are py_compile-clean and the alive-rule + `round_phase` npz round-trip were tested in isolation, but the v4 recorder/loader have NOT been run on Windows+CS2 yet. Confirmation steps: `python -m src.recorder --verify-gsi` (alive tracks life/death + spectating→dead), a short `--record` producing a real v4 session, then `inspect_recording` + a loader `--gameplay-filter` frame-count check. **`inspect_recording.py` was NOT updated for v4** — check it doesn't assert on schema version. The prior mid-recording v3 dataset is being deleted and re-recorded fresh as v4 (D-031).

**Alive-rule correction + format now v5 (2026-08):** two things changed after the initial build. (a) **D-032** corrected the own-POV/spectating tell from `observer_slot` (present in our own alive payload on this CS2, so it forced every in-game frame to `alive=False`) to the **steamid match** (`player.steamid == provider.steamid`); the fix was proven by replaying real logged payloads, and `--verify-gsi` now prints a coverage-aware verdict. On-machine, `alive=True` while playing is confirmed; the spectate→dead leg still needs a bot-match die+spectate to exercise. (b) **D-033** bumped the format **v4 → v5**, adding per-frame state features (health/weapon/ammo) as model inputs — see #12. Since the dataset was being re-recorded fresh anyway, it is now recorded as **v5**. `inspect_recording.py` HAS since been updated (v4 + v5 aware, with an alive summary and a state-feature summary), superseding the "NOT updated for v4" caveat above.
...
**State features already captured for this (2026-08, D-033):** the model INPUTS this sub-policy will most want — per-frame `health`, `active_weapon`, `ammo_clip`, `ammo_reserve` — are now recorded in every v5 session and exposed by `data_loader.load_state_features()` (a numeric `[health, ammo_clip, ammo_reserve]` matrix + the weapon string, sentinels preserved). They are STATE to condition on, NOT action labels, and are deliberately kept out of `get_batch`'s action `Y`. This is captured-ahead-of-need: nothing reads them yet, but they can only be collected live, so they ride along with the fresh dataset now. When #12 is built, "respect fire-rate / don't spray at nothing" can use clip-count and weapon directly instead of inferring them from pixels. This does NOT unblock #12 (still gated behind #10 detection and #11 aim) — it just means the inputs will be there.
